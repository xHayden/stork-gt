...
const APIForm = ({ requestUrl, requestMethod, fields, buttonText, description }: any) => {
    ...
    const getCurl: any = () => {
        ...
        return fetchToCurl(window.location.host + newRequestURL, options);
    }
            ...
            <div className="flex flex-col gap-4 justify-center pt-4">
                ...
                >
                    ...
                    { response ? <div className='border-white'>
                        <p className="w-full break-all text-white p-0 m-0 bg-neutral-800 no-scrollbar max-h-40 overflow-scroll" contentEditable={true} key={response}>{response}</p>
                    </div> :  <></> }
                </div>
        </div>
    );
